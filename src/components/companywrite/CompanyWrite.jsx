...
const CompanyWrite = () => {
  ...
  const getStacks = async () => {
    try {
      const repo = await axios.get(
        `http://54.180.112.137:9990/api/company/jobPost/page/stackList`,
        {
          // 직업군 카테고리 가져오기.
          headers: {
  ...
  const getImg = async () => {
    ...
    try {
      const repo = await axios.post(
        `http://54.180.112.137:9990/s3/file`,
        formData,
        {
          headers: {
  ...
  const getGroup = async () => {
    try {
      const repo = await axios.get(
        `http://54.180.112.137:9990/api/company/jobPost/page/jobGroup`,
        {
          // 직군 카테고리 1
          headers: {
            "Content-Type": "application/json",
  ...
  const getGroup2 = async () => {
    try {
      const repo = await axios.get(
        `http://54.180.112.137:9990/api/company/jobPost/page/jobDetail/${gotGroup1}`,
        {
          // 직군 카테고리 2
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 채용공고 올리기
      await axios.post(
        `http://54.180.112.137:9990/api/company/jobPost`,
        formData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="company-write">
        ...
        <div onClick={sendImage} className="company-btn">
          이미지 업로드
        </div>
        <h3>내용 작성</h3>
